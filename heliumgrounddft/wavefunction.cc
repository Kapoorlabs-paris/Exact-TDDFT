  ...
  for (xindex=0; xindex<g.ngps_x(); xindex++)
      ...
      for (yindex=0; yindex<g.ngps_y(); yindex++)
	{
	  ...
	  for (zindex=0; zindex<g.ngps_z(); zindex++)
		...
		{
		case 0 :
		  start[index_i]=(x+y)*exp(-0.01*x*x-0.01*y*y);
		  break;
		case 1 :
		  start[index_i]=y*sin(173*3.14*(x)/(g.ngps_x()))*exp(-width*y*y)+x*sin(173*3.14*(y)/(g.ngps_y()))*exp(-width*x*x);  //cos(x)*cos(y); //(1/sqrt(2))*(sin(x)*cos(y)+sin(y)*cos(x));//(sin(x)*cos(y)+sin(y)*cos(x)) ;   //cos(x)*cos(y)*exp(-x*x-y*y);//(sin(x)*cos(y)+sin(y)*cos(x))*exp(-x*x-y*y) ;
		  break;
		case 2 :
...
complex<double> wavefunction::trace(grid g,double box,const wavefunction &wf, const wavefunction &wfm)
{
	 dcomplex imagi;
    ...
    imagi=dcomplex(0.0,1.0);
	 ...
	 wavefunction density(g.ngps_x());
	  double x,y,z;
	 wavefunction rhs(g.ngps_y());
	 wavefunction lhs(g.ngps_y());
  zindex=0;
  density.nullify();
dcomplex result;
dcomplex resultlhs;
...
resultlhs+=conj(start[index])*wf[index]*g.delt_x()*g.delt_y();
...
resultrhs+=conj(start[index])*wfm[index]*g.delt_x()*g.delt_y();
...
result=(conj(resultlhs)*resultlhs)+(conj(resultrhs)*resultrhs);
...
/*
 ...
 for (xindex=0; xindex<g.offs_x()+box; xindex++)
   {
	      index=g.index(xindex,yindex,zindex);
	      density[xindex]=density[xindex]+real(conj(wf[index])*wf[index])*g.delt_y();
	    };
	};
 ...
 for (xindex=0; xindex<g.offs_x()+box; xindex++)
   {
	      index=g.index(xindex,yindex,zindex);
	      x=g.x(xindex);
	      rhs[yindex]+=start[index]*conj(sqrt(density[xindex])*exp(-imagi*k*x))*g.delt_x();//start[index]*conj(exp(-imagi*k*x))*g.delt_x();//start[index]*conj(sqrt(density[xindex])*exp(-imagi*k*x))*g.delt_x();
	    };
	};
   ...
   for (xindex=0; xindex<g.offs_x()+box; xindex++)
   {
	      index=g.index(xindex,yindex,zindex);
	       x=g.x(xindex);
	      lhs[yindex]+=exp(-imagi*k*x)*sqrt(density[xindex])*conj(start[index])*g.delt_x();//exp(-imagi*k*x)*conj(start[index])*g.delt_x();//exp(-imagi*k*x)*sqrt(density[xindex])*conj(start[index])*g.delt_x();
	    };
	};
  ...
  
   {
	      index=g.index(xindex,yindex,zindex);
	       x=g.x(xindex);
	      result+=lhs[yindex]*rhs[yindex]*g.delt_x();
	    };
	};
...
complex<double> wavefunction::tracenode(grid g,double box,const wavefunction &wf)
{
	 dcomplex imagi;
...
result=(conj(resultlhs)*resultlhs);
...
complex<double> wavefunction::traceion(grid g,double box,const wavefunction &wf,double k)
{
	 dcomplex imagi;
...
wavefunction wavefunction::staticpotential(grid g, const wavefunction &dens)
{
  wavefunction rhs(g.ngps_x());
 wavefunction lhs(g.ngps_x());
  wavefunction timediff(g.ngps_x());
...
zindex=0;
  ...
  for (xindex=0; xindex<g.ngps_x(); xindex++)
	...
	for (xindex=0; xindex<g.ngps_x(); xindex++)
    {
	alphapart[xindex]=0.5*spacediff[xindex];
   };
...
wavefunction wavefunction::phasekohnshamorbital(grid gone, grid g, const wavefunction &wf)
{
...
wavefunction result_y(g.ngps_y());
 ...
 wavefunction upper(g.ngps_x());
  ...
  double lambda=sqrt(3.0)-2.0;
  double llambda=-sqrt(3.0)+2.0;
  dcomplex thetaoffset,thetaboundary;
  int n;
double x, y, z;
...
for (xindex=0; xindex<g.ngps_x(); xindex++)
  ...
  for (xprimeindex=0; xprimeindex<=xindex; xprimeindex++)
    {
      thetaoffset=thetaoffset+((current[xprimeindex])/((density[xprimeindex]+fcteps)))*g.delt_x();
    };
  ...
  for (xindex=0; xindex<g.ngps_x(); xindex++)
    {
      for (xprimeindex=0; xprimeindex<=xindex; xprimeindex++)
	{
	  theta[xindex]=theta[xindex]+((current[xprimeindex])/((density[xprimeindex]+fcteps)))*g.delt_x();
  ...
  // ---------- W_-x
		 ...
		 for (xindex=0; xindex<g.ngps_x(); xindex++)
	{
	  index=g.index(xindex,yindex,zindex);
		  ...
		  thepot[index]=-0.5*(rhsone[index])/(density[index]+fcteps);
